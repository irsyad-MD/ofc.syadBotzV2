    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          //if(!user.owner || !user.rowner) return conn.reply(m.chat, `Maaf @${m.sender.split`@`[0]} \n*『 Info 』*\nKami Staff irsyad ingin melakukan backup data, dan sedikit meningkatkan performa bot.\nJadi, Kami dari Staff irsyad memutuskan untuk melakukan *maintenance*, jika ada yang mau ditanyakan, harap hubungi Real Owner, Terima Kasih\n\n_*Hormat Kami, Staff irsyad*_`, m, { mentions: [m.sender] })
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.sendButton(m.chat, `Limit anda habis, silahkan chat Owner atau beli melalui *${usedPrefix}buy*`, '© ofc.syadBotzV2𝐭', { 'button[0]': 'Owner', 'row[0]': '.owner', 'button[1]': 'Buy', 'row[1]': '.buy', 'button[2]': 'Buy All', 'row[2]': '.buyall' }, m)
